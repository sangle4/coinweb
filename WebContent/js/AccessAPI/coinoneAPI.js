function writeTableCoinone(coin){
	$.get('https://api.coinone.co.kr/ticker?currency=all', function(data) {
		...
		try {
		    ...
		    $('#coinone_volume').html(coinone_coin_trade.toFixed(1) + " " + coin.toUpperCase()); //1일 거래량
			...
			errorThrow("Coinone", coin);
			
			$('#coinone_KRW').html(" - "); //실시간시세 원화
		 	$('#coinone_USD').html(" - "); //실시간시세 달러
		    $('#coinone_volume').html(" - "); //1일 거래량
		    document.getElementById('coinone_fluctate').style.color = "black";
		    $('#coinone_fluctate').html(" - ");
		    calculPremium(coinone_coin, "coinone", "null")
		}
	});
}